...
bool GuSprite_LoadSprite(char *FileName,struct SGuSprite *sGuSprite,unsigned char alpha)
{
 ...
 //определяем размер изображения, являющегося степенью двойки с шириной, кратной 4-м байтам
...
//отобразить спрайт
void GuSprite_PutSpriteElement(int x,int y,int xb,int yb,int width,int height,struct SGuSprite *sGuSprite)
{
 if (sGuSprite->Data==NULL || sGuSprite->Width==0 || sGuSprite->Height==0) return;//спрайт отсутствует
 sceGuTexMode(GU_PSM_8888,0,0,0);
 sceGuTexImage(0,sGuSprite->WidthImage,sGuSprite->HeightImage,sGuSprite->WidthImage,sGuSprite->Data);
 ...
 if (xe<0) return;
 if (ye<0) return;
 if (xb>sGuSprite->Width) return;
 if (yb>sGuSprite->Height) return;
 if (xe>sGuSprite->Width) xe=sGuSprite->Width;
 if (ye>sGuSprite->Height) ye=sGuSprite->Height;
 ...
 sGuVertex[1].X=x+width;
 sGuVertex[1].Y=y+height;
 sGuVertex[1].Z=0;
 sGuVertex[1].Color=0xffffffff;
 sGuVertex[1].U=xe;
 sGuVertex[1].V=ye;
